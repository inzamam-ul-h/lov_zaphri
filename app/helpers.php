...
function getAppName() {
    ...
    $appNameSetting = config('app.name');
...
function getThumbLogoUrl() {
    ...
    //$setting = Setting::where('key', '=', 'logo_url')->first();
    //$thumbLogo = (! empty($setting) && ! empty($setting->value)) ? app(Setting::class)->getLogoUrl($setting->value, Setting::THUMB_PATH) : asset('assets/images/logo-30x30.png');
    $thumbLogo = asset('assets/images/logo-30x30.png');
...
function getFaviconUrl() {
    ...
    $favicon = asset('assets/images/favicon/favicon-16x16.ico');
