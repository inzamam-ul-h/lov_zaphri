...
class ChatRepository extends BaseRepository {
     ...
     * @return array
     */
    public function getLatestConversations($input = [], $User = NULL, $Listing = 'both') {
        if ($User === NULL) {
            $User = Auth::user();
        }
        ...
        $archiveUsers = ArchivedUser::toBase()
                        ->select('owner_id')
                        ->whereArchivedBy($this->getAuthUser()->id)
        ...
        if (!$isArchived) {
            ...
            $archiveGroups = [];
            if ($isGroupChatEnabled) {
                $archiveGroups = ArchivedUser::whereArchivedBy($this->getAuthUser()->id)->whereOwnerType(Group::class)->get()->pluck('owner_id')->toArray();
            }
        ...
        foreach ($groupsConversation as $conversation) {
            ...
            if (!isset($lastConversations[$this->getAuthUser()->id])) {
                continue;
            }
            $newConversation = $lastConversations[$this->getAuthUser()->id]->conversation;
            ...
            $chatList[$conversation->id]['group_details'] = $lastConversations[$this->getAuthUser()->id]->group_details;
        }

        return array_values($chatList);
    }

    public function getLatestGroupConversations($input = [], $User = NULL, $Listing = 'both') {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        if ($isGroupChatEnabled) {
            ...
            $bindings = array_merge($groupSubQuery->getBindings()); //$subQuery->getBindings(),
            $groupSubQueryStr = $groupSubQuery->toSql();
            if ($isUserChatEnabled) {
                $relations = ['group.lastConversations.conversation', 'user.userStatus', 'group.usersWithTrashed'];
            }
            else {
                $relations = ['group.lastConversations.conversation', 'group.usersWithTrashed'];
            }
        }
        elseif ($isUserChatEnabled) {
            $bindings = $subQuery->getBindings();
            $relations = ['user.userStatus'];
        }
        if ($isGroupChatEnabled && $isUserChatEnabled) {
            $subQueryStr = $subQuery->toSql();
        }
    ...
    public function sendMessage($input, $User = NULL) {
        ...
        if (isset($input['is_archive_chat']) && $input['is_archive_chat'] == 1) {
            $archivedUser = ArchivedUser::whereOwnerId($input['to_id'])->whereArchivedBy($this->getAuthUser()->id)->first();
        ...
        $input['from_id'] = $this->getAuthUser()->id;
        ...
        if (isset($input['is_group']) && $input['is_group']) {
            return $this->sendGroupMessage($input, $User);
        }
        ...
        if ($receiver->is_subscribed && !$receiver->devices->isEmpty()) {
            // Send OneSignal Push of user is subscribed
            $headings = $receiver->name . ' | ' . getAppName();

            foreach ($receiver->devices as $device) {
                $options = [];
                if ($conversation->message_type == 1) {
                    $options['image'] = $conversation->message;
                }
                dispatch(new SendOneSignalPushJob([$device->player_id], $headings, $input['message'], $options));
            }
        }
        ...
        $notificationRepo->sendNotification($notificationInput, $conversation['to_id'], 0, $User);
     ...
     * @return bool
     */
    public function groupMessageValidations($groupUsers, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        if (!in_array($this->getAuthUser()->id, $groupUsers)) {
     ...
     * @return Conversation
     */
    public function sendGroupMessage($input, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        $this->groupMessageValidations($groupUsers, $User);

        /** @var $conversation Conversation */
        $conversation = $this->create($input)->fresh();
        $conversation->sender;
        $broadcastData = $this->prepareGroupChatBroadCastData($conversation->toArray(), $group->toArray(), $User);
        if (!empty($conversation->reply_to)) {
            $broadcastData['reply_message']['id'] = $conversation->replyMessage->id;
            $broadcastData['reply_message']['message'] = $conversation->replyMessage->message;
            $broadcastData['reply_message']['sender']['id'] = $conversation->replyMessage->sender->id;
        ...
        $this->groupRepo->addRecordsToGroupMessageRecipients($groupUsers, $conversation->id, $group->id, $User);
        ...
        if (count($playerIds)) {
            ...
            dispatch(new SendOneSignalPushJob($playerIds, $headings, $input['message'], $options));
        }
        $conversation->readBy;

        $this->sendGroupMessageNotification($conversation, $group->users, $User);
     ...
     */
    public function sendGroupMessageNotification($conversation, $groupMembers, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        foreach ($groupMembers as $groupMember) {
            if ($groupMember->id == $this->getAuthUser()->id) {
                continue;
            }
            $notificationInput = [
            ...
            /** @var NotificationRepository $notificationRepo */
            $notificationRepo = app(NotificationRepository::class);
            $notificationRepo->sendNotification($notificationInput, $groupMember->id, 0, $User);
        }
    }
     ...
     * @return mixed
     */
    public function prepareGroupChatBroadCastData($conversation, $group, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
     ...
     */
    public function addAttachment($file, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
     ...
     * @return array
     */
    public function markMessagesAsRead($input, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);

        $senderId = $this->getAuthUser()->id;
        $isGroup = (isset($input['is_group']) && $input['is_group'] == '1') ? true : false;
        $remainingUnread = 0;

        if ($isGroup && !empty($input['ids'])) {
            $this->readGroupMessages($input['ids']);
            $remainingUnread = $this->getUnreadMessageCount($input['group_id'], true, $User);
        }
        elseif (!$isGroup && !empty($input['ids'])) {
            ...
            $senderId = ($conversation->from_id == $this->getAuthUser()->id) ? $conversation->to_id : $conversation->from_id;
            $receiverId = ($conversation->from_id == $this->getAuthUser()->id) ? $conversation->from_id : $conversation->to_id;
            $remainingUnread = $this->getUnreadMessageCount($senderId, false, $User);
     ...
     * @return bool
     */
    public function readGroupMessages($ids, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        if (!empty($readMessages)) {
            broadcast(new GroupEvent(
                        ...
                        'read_by_user_id'  => $this->getAuthUser()->id,
     ...
     * @return int
     */
    public function getUnreadMessageCount($senderId, $isGroup = false, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        if ($isGroup) {
            return GroupMessageRecipient::whereUserId($this->getAuthUser()->id)->where('group_id', $senderId)
                            ->whereNull('read_at')->count();
        }

        return Conversation::where(function (Builder $q) use ($senderId) {
                    $q->where('from_id', '=', $senderId)->where('to_id', '=', $this->getAuthUser()->id);
     ...
     * @param $userId
     */
    public function deleteConversation($userId, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        $chatIds = Conversation::leftJoin('message_action as ma', function (JoinClause $join) {
                ...
                ->where(function (Builder $q) use ($userId) {
                    $q->where(function (Builder $q) use ($userId) {
                        ...
                        ->where('to_id', '=', $this->getAuthUser()->id);
                    })->orWhere(function (Builder $q) use ($userId) {
                        $q->where('from_id', '=', $this->getAuthUser()->id)
                        ->where('to_id', '=', $userId);
                    });
                })
                ->where(function (Builder $q) {
        ...
        foreach ($chatIds as $chatId) {
            $input[] = [
                ...
                'deleted_by'      => $this->getAuthUser()->id,
            ];
        }
     ...
     * @return bool
     */
    public function deleteGroupConversation($groupId, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
     ...
     * @param $id
     */
    public function deleteMessage($id, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
     ...
     * @throws Exception
     */
    public function sendChatRequest($input, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        $toId = $input['to_id'];
        $message = $input['message'];
        $chatRequest = ChatRequestModel::whereFromId($User->id)
                ->whereOwnerId($toId)
        ...
        ChatRequestModel::whereFromId($User->id)
                ->whereOwnerId($toId)
     ...
     */
    public function sendAcceptDeclineChatRequestNotification($chatRequest, $userEventType = 0, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
        ...
        /** @var NotificationRepository $notificationRepo */
        $notificationRepo = app(NotificationRepository::class);
        $notificationRepo->sendNotification($notificationInput, $chatRequest['from_id'], $userEventType, $User);
    }
