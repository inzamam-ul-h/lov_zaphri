...
class UserRepository extends BaseRepository {
    ...
    protected $fieldSearchable = [
        'name', 'email', 'phone', 'last_seen', 'about', 'photo_url',
    ]; //, 'is_online'
     ...
     * @return array
     */
    public function getConversation($id, $input, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        ...
        if ($isGroup) {
            ...
            /** @var Group $group */
            $group = Group::with([
                        'users.userStatus', 'lastConversations', 'createdByUser', //'users.roles', 'usersWithTrashed', 
                    ])->find($id);
            ...
            if ($group->removed_from_group) {
                ...
                if (isset($lastConversation[$this->getAuthUser()->id])) {
                    $lastConversation = $lastConversation[$this->getAuthUser()->id];
            ...
            /** @var User $user */
            $user = User::with(['roles', 'userStatus', 'reportedUser'])->find($id);
            if (empty($user)) {
                throw new BadRequestHttpException('User not found.');
            }

            $isPrivateAccount = ($user->privacy == 0) ? true : false;
            $isSuperAdmin = $user->hasRole('admin'); //Role::ADMIN_ROLE_NAME
            $isMyContact = $this->checkIsMyContact($id, $User);
            ...
            $commonGroups = Group::with(['users' => function ($q) {
                                    $q->select(['users.name', 'users.id']);
                                }])->whereHas('users', function ($q) use ($id, $authUser) {
                                return $q->whereIn('users.id', [$id, $authUser->id]);
                            }, '>=', 2)
                            ->get()->toArray();

            $user = $user->toArray();
            /* $SITE_URL = env('APP_URL');
              $defaultImage = $SITE_URL."/uploads/default/user.png";
              $uploadsPath = $SITE_URL."/uploads/users/".$id; */
            $photo_url = $user['photo'];
            /* if(!empty($photo_url) && $photo_url != 'default_image' && $photo_url != 'user.png')
              $photo_url = $uploadsPath. "/" . $photo_url;
              else
            ...
            if (!empty($blockedUser) && isset($blockedUser[$this->getAuthUser()->id])) {
            ...
            $user['common_groups'] = $commonGroups;
        }
        $allMedia = [];
        ...
        if ($isGroup) {
            ...
            $query->with(['sender', 'readBy']); //, 'receiver'
        }
        else {
        ...
        $unreadCount = $countQuery->where('conversations.status', '=', 0)->where('to_id', '=', $this->getAuthUser()->id)->count();
        if (!isset($input['before']) && !isset($input['after'])) {
            $allMedia = $mediaQuery->whereIn('message_type', Conversation::MEDIA_MESSAGE_TYPES
                    )->get(['conversations.*']);
        }

        $oldest_query = clone $query;
        $latest_query = clone $query;
        $oldest_messages = $oldest_query->orderBy('conversations.id', 'ASC')->limit(1)->get(['conversations.id'])->toArray();
        $latest_messages = $latest_query->orderBy('conversations.id', 'DESC')->limit(1)->get(['conversations.id'])->toArray();
    ...
    public function updateProfilePhoto(User $user, $input) {
        try {
            ...
            $input['is_subscribed'] = isset($input['is_subscribed']) ? 1 : 0;
            $user->update($input);
    ...
    public function setUserCustomStatus($input) {
        $input['user_id'] = $this->getAuthUser()->id;
        $input['emoji'] = str_replace($input['emoji_short_name'], '', $input['emoji']);
        $userStatus = UserStatus::where('user_id', '=', $this->getAuthUser()->id)->first();
        if (!empty($userStatus)) {
    ...
    public function clearUserCustomStatus() {
        UserStatus::whereUserId($this->getAuthUser()->id)->delete();

        $broadcastData['type'] = User::STATUS_CLEAR;
        $broadcastData['user_id'] = $this->getAuthUser()->id;
        broadcast(new UpdatesEvent($broadcastData))->toOthers();
    }
     ...
     * @return array
     */
    public function myContactIds($User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);
     ...
     * @return bool
     */
    public function checkIsMyContact($userId, $User = NULL) {
        if ($User === NULL) {
            $User = Auth::user();
        }
        $this->setAuthUser($User);

        $myContactIds = $this->myContactIds($User);
    ...
    public function deleteUser($userId) {
        try {
            ...
            User::whereId($userId)->forceDelete();
            DB::commit();
        }
