...
class MainController extends Controller {
    ...
    protected function get_user_array($User, $refresh_token = FALSE) {
        ...
        if (!empty($User)) {
            ...
            //$status = $User->status;
            $created_by = $User->created_by; //hghhhhhhhhhh
            $first_name = '';
            $last_name = '';
            $about_me = '';
            $zip_code = '';
            ...
            $data = [
                ...
                'user_type_label' => (string)$user_type_label,
                ...
                'phone_prefix'    => (string)$phone_prefix,
                'profile_status'  => (string)$profile_status,
                'about_me'        => (string)$about_me,
                'zip_code'        => (string)$zip_code,
                'timezone_id'     => (string)$timezone,
                'gender'          => (string)$gender,
                'coachpic'        => (string)$coachpic,
                'created_by'      => (string)$created_by,
                'fcm_token'       => (string)$fcm_token,
            ];

            if ($user_type == $this->_COACH_USER) {
                $data["meetinglink"] = (string)$res_profile->meetinglink;
                $data["public_url"] = env('APP_URL') . '/' . get_user_data('public_url', $user_id);
                $data["url_status"] = (string)$User['url_status'];
            }
            if ($user_type == $this->_PLAYER_USER) {
                $data["dob"] = (string)$res_profile->dob;
                $data["club"] = (string)$res_profile->dob;
            }
            if ($user_type == $this->_CLUB_USER) {
                $data["reg_no"] = (string)$res_profile->reg_no;
                $data["contact_person"] = (string)$res_profile->contact_person;
                $data["address"] = (string)$res_profile->address;
            }
