...
class SessionApiController extends BaseController
{
        ...
        if ($result['status']) {
            ...
            switch ($action) {
                ...
                case 'search_session': {
                    ...
                    break;
                    // book multiple seession 
                case 'book_sessions': {
                        return $this->book_sessions($request, $User);
                    }
                    break;
    ...
    private function search_session(Request $request, $User)
    {
        ...
        $availabilities = DB::table('sessions as s')
        ->where('s.time_start', '>=', $start) // Filter by session start time
        ->where('s.time_end', '<=', $end)    // Filter by session end time
        ->where('s.booked', 0)              // Only include sessions that are not booked
        ->whereNotExists(function ($existsQuery) use ($user_id) {
            $existsQuery->select(DB::raw(1))
                ->from('bookings as b')
                ->whereRaw('b.session_id = s.id') // Match session ID
                ->where('b.req_user_id', $user_id)    // Check for the specific user
                ->whereIn('b.status', [1, 3]);  // Exclude booked (status = 1) or canceled (status = 3) sessions
        })
        ->groupBy('s.id')                // Ensure each session appears only once
        ->orderBy('s.time_start', 'asc') // Sort by session start time
        ->limit($limit)
        ->offset($offset)
        ->get();
    ...
    private function book_session(Request $request, $User)
    {
        ...
        public function book_sessions(Request $request, $User)
         {
        $user_id = $User->id;
        $user_type = $User->user_type;
        ...
        if (!isset($request->sessions) || !is_array($request->sessions)) {
            return $this->sendError('Invalid sessions data. Provide an array of sessions.');
        }

        $current = strtotime(date('Y-m-d H:i'));
        $response = []; // To hold the success and error messages for each session
        $hasApproval = false; // Track if any session requires approval
        $allSuccess = true; // Assume all are successful initially

        foreach ($request->sessions as $session) {
            $session_id = trim($session['session_id'] ?? '');
            $session_type = trim($session['session_type'] ?? '');

            if (empty($session_id) || empty($session_type)) {
                $response[] = [
                    'session_id' => $session_id,
                    'status' => 'error',
                    'message' => 'Missing session parameters.',
                ];
                $allSuccess = false;
                continue;
            }

            $ModelData = Session::where('id', $session_id)
                ->where('status', 1)
                ->where('booked', 0)
                ->first();
            ...
            if (empty($ModelData)) {
                $response[] = [
                    'session_id' => $session_id,
                    'status' => 'error',
                    'message' => 'Invalid or unavailable session provided.',
                ];
                $allSuccess = false;
                continue;
            }
            ...
            if ($time_start < $current) {
                $response[] = [
                    'session_id' => $session_id,
                    'status' => 'error',
                    'message' => 'You cannot book past sessions.',
                ];
                $allSuccess = false;
                continue;
            }
            ...
            switch ($bool) {
                case 1: // Success
                    $ModelData->type = $session_type;
                    $ModelData->booked = 1;
                    $ModelData->save();
            
                    $Booking = new Booking();
                    $Booking->user_id = $ModelData->user_id;
                    $Booking->req_user_id = $user_id;
                    $Booking->session_id = $session_id;
                    $Booking->status = 1;
                    $Booking->save();
                    $response[] = [
                        "status"=>'success',
                        "session_id"   => $session_id,
                        "date"         => date('M d, Y', $ModelData->time_start),
                        "start_time"   => date('h:i A', $ModelData->time_start),
                        "session_type" => $ModelData->type,
                        "coach_id"     => $ModelData->user_id,
                        "coach"        => get_user_name($ModelData->user_id),
                        "rating"       => get_user_data('rating', $ModelData->user_id),
                ...
                case 2: // Waiting for Approval
                    $response[] = [
                        'session_id' => $session_id,
                        'status' => 'error',
                        'message' => "Waiting for approval [" . date('M d, Y H:i A', $time_start) . " - " . date('H:i A', $time_end) . "]",
                    ];
                    $hasApproval = true;
                    $allSuccess = false;
                    break;
                ...
                case 3: // Already Booked
                    $response[] = [
                        'session_id' => $session_id,
                        'status' => 'error',
                        'message' => "Already booked [" . date('M d, Y H:i A', $time_start) . " - " . date('H:i A', $time_end) . "]",
                    ];
                    $allSuccess = false;
                    break;

                case 4: // Booking Declined
                    $response[] = [
                        'session_id' => $session_id,
                        'status' => 'error',
                        'message' => "Booking declined [" . date('M d, Y H:i A', $time_start) . " - " . date('H:i A', $time_end) . "]",
                    ];
                    $allSuccess = false;
                    break;
        ...
        // Final Response
        if ($allSuccess) {
            return $this->sendSuccess('All sessions booked successfully.', $response);
        }

        if ($hasApproval || !$allSuccess) {
            return $this->sendSuccess('One or more sessions waiting for approval.', $response);
        }

        return $this->sendError('Some sessions could not be booked.');
    }
