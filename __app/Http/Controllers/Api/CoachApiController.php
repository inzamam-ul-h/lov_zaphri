    ...
    private function coach_my_sessions(Request $request, $User) {
        ...
        if ($user_type == $this->_COACH_USER) {
            ...
            if ($listing == "upcoming" || $listing == "both") {
                ...
                $sessions = $query->select('sessions.id', 'sessions.user_id', 'sessions.type', 'sessions.price', 'sessions.color', 'sessions.booked', 'sessions.time_start', 'sessions.time_end', 'bookings.session_id', 'bookings.id as booking_id', 'bookings.req_user_id', 'bookings.status', 'sessions.status as session_status', 'bookings.status as booking_status')
                        ->orderBy('sessions.time_start', 'asc')
                        ->limit($limit)
                        ->offset($offset)
                        ->get();
